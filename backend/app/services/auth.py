...
def create_magic_link(member_id: str, token: str) -> str:
    settings = get_settings()
    s = _get_serializer()
    signed = s.dumps({"token": token, "member_id": str(member_id)})
    return f"{settings.base_url}/auth?t={signed}"


def verify_magic_link(signed_token: str, max_age: int = 86400) -> dict:
    """Verify a signed magic link token. Returns {"token": ..., "member_id": ...}."""
    s = _get_serializer()
    try:
